...
wilcox.test(mini_cus_other$other_seq_reads ~ mini_cus_other$db ,paired=TRUE) # not rejected
...
# alternative hypothesis, cus db with significantly lesser reads
# since tested on the same data set with different methods, paired in true
wilcox.test(mini_cus_unc$unc_reads ~ mini_cus_unc$db ,paired=TRUE) # not rejected
...
# alternative hypothesis, cus db with significantly lesser reads
# since tested on the same data set with different methods, paired in true
wilcox.test(mini_std_unc$unc_reads ~ mini_std_unc$db,paired=TRUE) # not rejected
...
# mini vs custom for other sequences reads
mini_cus_mic <- data.frame(db=c(rep("cus",12),rep("std",12)), mic_reads=c(cus_micro_comp$sum_reads,std_micro_comp$sum_reads))
# wilcoxon test requires exactly 2 levels of groups factors
mini_cus_mic$db <- factor(mini_cus_mic$db, levels=c("cus","std"))

# wilcoxon test 
# alternative hypothesis, cus db with significantly lesser reads
# since tested on the same data set with different methods, paired in true
wilcox.test(mini_cus_mic$mic_reads ~ mini_cus_mic$db,paired=TRUE) # not rejected
