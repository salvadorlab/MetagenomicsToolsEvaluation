...
for file in $data_path/*; do
     ...
     $path/CLARK/CLARKSCV1.2.6.1/classify_metagenome.sh -P $data_path/$sample/${sample}_1_kneaddata_paired_1.fastq $data_path/$sample/${sample}_1_kneaddata_paired_2.fastq -R $path/CLARK/output/bacteria_phylum/$sample.txt
done
...
for file in /scratch/rx32940/CLARK/output/*; do 
    ...
    $path/CLARK/CLARKSCV1.2.6.1/estimate_abundance.sh -F /scratch/rx32940/CLARK/output/bacteria_phylum/$sample_csv.txt.csv -D $path/CLARK/DB > /scratch/rx32940/CLARK/output/bacteria_phylum/${sample_csv}_abundance.txt
done
