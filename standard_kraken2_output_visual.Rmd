...
plot <- ggplot(phylum_all_samples, aes(x=samples,y=Reads,fill=Phylum)) +
  geom_bar(stat="identity") + 
  labs(title = "Metagenomic Reads Classification with Standard Kraken2 DB (Phylum)", y="Number of Reads", x = "Samples", caption = "* For the purpose of visualization, only taxa classified with more than 100 reads will be shown") + 
  theme(axis.title = element_text(size=1), axis.text.x = element_text(angle=90), legend.key.size =unit(0.5,"cm")) 
plot
...
phylum_all_samples %>%
  group_by(samples) %>%
  mutate(Total=sum(Reads)) %>%
  mutate(Relative=Reads/Total) ->phylum_relative


relative_plot <- ggplot(phylum_relative, aes(x=samples,y=Relative,fill=Phylum)) +
  geom_bar(stat="identity") + 
  labs(title = "Metagenomic Relative Abundance with Standard Kraken2 DB (Phylum)", y="Percentage", x = "Samples", caption = "* For the purpose of visualization, only taxa classified with more than 100 reads will be shown") + 
  theme(axis.title = element_text(size=1), axis.text.x = element_text(angle=90), legend.key.size =unit(0.5,"cm")) +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(axis.text.x=element_text(angle=90)) + 
