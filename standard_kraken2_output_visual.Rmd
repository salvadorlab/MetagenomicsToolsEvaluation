...
ggsave("/Users/rachel/Dropbox/5.Rachel-projects/Metagenomic_Analysis/final_analysis/kraken2/Plots/Absolute/kraken2_std_domain.png",width=8, height=6)
...
ggsave("/Users/rachel/Dropbox/5.Rachel-projects/Metagenomic_Analysis/final_analysis/kraken2/Plots/Relative/kraken2_std_domain_rel.png", width=8, height=6)
...
### Classify genus, with Homo, keeping on taxa with more than 10% in composition
```{r}
absolute_path = paste(path,"/absolute/",sep="")
files <- list.files(absolute_path)
...
genus_all_samples %>% group_by(samples) %>%subset(genus != "Rattus") %>% summarise(sum=sum(Reads)) 
genus_all_samples
```

```{r}
manual_color <-c("#54478C","#F94144","#80B918","#FEE440","#4361EE","#FFADAD","#FFD6A5","#FDFFB6","#CAFFBF","#9BF6FF","#A0C4FF","#BDB2FF","#FFC6FF","#233D4D","#FE7F2D","#FCCA46","#A1C181",
                 "#619B8A","#EE6352","#59CD90","#3FA7D6","#FAC05E","#F79D84","#50514F","#F25F5C","#FFE066","#247BA0","#70C1B3",
                 "#FFD1E8","#C8A439","#465D6F","#A94303","#708841","#540D6E","#EE4266","#FFD23F","#3BCEAC","#0EAD69","#E3AB00ff","#D1DED3ff",
                 "#AD8EB0ff","#58BADCff","#A71D2Eff","#86D8BBff","#0050ADff","#97CBF0ff","#67A280ff","#F20089","#41EAD4","#FBFF12","#54478C")
...
plot <- ggplot(major_genus, aes(x=samples,y=Reads,fill=genus)) +
  geom_bar(stat="identity") + 
  labs(title = "Metagenomic Reads Classification with Standard Kraken2 DB (Genus)", y="Number of Reads", x = "Samples") + 
...
relative_plot <- ggplot(major_genus, aes(x=samples,y=Percentage,fill=genus)) +
  geom_bar(stat="identity") + 
  labs(title = "Metagenomic Relative Abundance with Standard Kraken2 DB (Genus)", y="Percentage", x = "Samples") + 
  theme(legend.text = element_text(size =8 ),legend.key.size =unit(0.4,"cm")) +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(axis.text.x=element_text(angle=90)) + 
  scale_fill_manual(values=manual_color)
