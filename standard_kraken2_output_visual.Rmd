...
domain_all_samples %>% subset(Domain == "Eukaryota" | Domain == "Bacteria" | Domain == "Viruses" | Domain == "Archaea") -> other_sequences_std
other_sequences_std %>% group_by(samples) %>% summarise(total_microbial=sum(Reads)) #%>%summarise(mean_other=mean(Reads),std=sd(Reads)) -> other_seq_std_summary
...
genus_all_samples %>% group_by(samples) %>%
  mutate(Total=sum(Reads), Percentage = Reads/Total) %>%subset( genus !="Homo") %>% summarise(sum_reads=sum(Reads), sum_percentage=sum(Percentage)) -> std_micro_comp
std_micro_comp
```
